# GitReward — security audit

It has **two parts** — both matter, and a contract audit alone does not cover the second:
...
| Token | USDC (6 decimals), set immutably at deploy |
...
2. **Owner key** — can `setOracleKey` to an attacker key, then
...
- The deployed contract is the source of truth; this behaviour is documented in
...
- No KMS/HSM or signer isolation, so a full
  ...
  bounds this to `[fund, expiry)` per bounty).
